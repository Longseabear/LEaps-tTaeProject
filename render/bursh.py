...
class Brush():
    def __init__(self, alpha=(1, 1), key='random'):
        if key=='random':
        ...
        self.alpha = np.stack([np.linspace(alpha[0], alpha[1], self.h), ] * self.w).transpose()

    def sample_p(self, t0, t1):
        return np.float32([[t0*self.h, 0], [t0*self.h, self.w], [self.h*t1, 0], [self.h*t1, self.w]])

    def sample(self, t0, t1):
        return self.img[int(t0 * self.h) : int(t1* self.h), :] * self.alpha[int(t0 * self.h) : int(t1* self.h), :]
