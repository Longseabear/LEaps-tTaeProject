import sys
sys.path.append("./ICCV2019-LearningToPaint/baseline")
...
from drawfile_format import DrawFileFormat
...
parser.add_argument('--title', default="test", type=str, help='test_title')
parser.add_argument('--output_folder', default="drawfile_output", type=str, help='output folder')
...
def decode(x: torch.tensor, canvas): # b * (10 + 3)
    x = x.view(-1, 10 + 3)
...
file_format = DrawFileFormat(args.title, origin_shape[0], origin_shape[1], args.img, args.output_folder)
...
with torch.no_grad():
    ...
    for i in range(args.max_step):
        ...
        temp = actions.view(-1, 10 + 3)
        for j in range(temp.shape[0]):
            action_out = [1, 0, 0] + list(temp[j].numpy())
            file_format.action_list.append(action_out);
    ...
    if args.divide != 1:
        ...
        for i in range(args.max_step):
            ...
            temp = actions.view(-1, 10 + 3)
            for j in range(temp.shape[0]):
                action_out = [args.divide, (j//5) // args.divide, (j//5) % args.divide] + list(temp[j].numpy())
                file_format.action_list.append(action_out);
    ...
    file_format.final_output_path = 'output/generated' + str(args.imgid-1) + '.png';
    file_format.save()
